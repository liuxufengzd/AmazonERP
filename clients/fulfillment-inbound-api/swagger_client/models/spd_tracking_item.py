# coding: utf-8

"""
    The Selling Partner API for FBA inbound operations.

    The Selling Partner API for Fulfillment By Amazon (FBA) Inbound. The FBA Inbound API enables building inbound workflows to create, manage, and send shipments into Amazon's fulfillment network. The API has interoperability with the Send-to-Amazon user interface.  # noqa: E501

    OpenAPI spec version: 2024-03-20
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


import pprint
import re  # noqa: F401

import six


class SpdTrackingItem(object):
    """NOTE: This class is auto generated by the swagger code generator program.

    Do not edit the class manually.
    """

    """
    Attributes:
      swagger_types (dict): The key is attribute name
                            and the value is attribute type.
      attribute_map (dict): The key is attribute name
                            and the value is json key in definition.
    """
    swagger_types = {
        'box_id': 'str',
        'tracking_id': 'str',
        'tracking_number_validation_status': 'str'
    }

    attribute_map = {
        'box_id': 'boxId',
        'tracking_id': 'trackingId',
        'tracking_number_validation_status': 'trackingNumberValidationStatus'
    }

    def __init__(self, box_id=None, tracking_id=None, tracking_number_validation_status=None):  # noqa: E501
        """SpdTrackingItem - a model defined in Swagger"""  # noqa: E501

        self._box_id = None
        self._tracking_id = None
        self._tracking_number_validation_status = None
        self.discriminator = None

        if box_id is not None:
            self.box_id = box_id
        if tracking_id is not None:
            self.tracking_id = tracking_id
        if tracking_number_validation_status is not None:
            self.tracking_number_validation_status = tracking_number_validation_status

    @property
    def box_id(self):
        """Gets the box_id of this SpdTrackingItem.  # noqa: E501

        The ID provided by Amazon that identifies a given box. This ID is comprised of the external shipment ID (which is generated after transportation has been confirmed) and the index of the box.  # noqa: E501

        :return: The box_id of this SpdTrackingItem.  # noqa: E501
        :rtype: str
        """
        return self._box_id

    @box_id.setter
    def box_id(self, box_id):
        """Sets the box_id of this SpdTrackingItem.

        The ID provided by Amazon that identifies a given box. This ID is comprised of the external shipment ID (which is generated after transportation has been confirmed) and the index of the box.  # noqa: E501

        :param box_id: The box_id of this SpdTrackingItem.  # noqa: E501
        :type: str
        """
        if box_id is not None and len(box_id) > 1024:
            raise ValueError("Invalid value for `box_id`, length must be less than or equal to `1024`")  # noqa: E501
        if box_id is not None and len(box_id) < 1:
            raise ValueError("Invalid value for `box_id`, length must be greater than or equal to `1`")  # noqa: E501

        self._box_id = box_id

    @property
    def tracking_id(self):
        """Gets the tracking_id of this SpdTrackingItem.  # noqa: E501

        The tracking ID associated with each box in a non-Amazon partnered Small Parcel Delivery (SPD) shipment.  # noqa: E501

        :return: The tracking_id of this SpdTrackingItem.  # noqa: E501
        :rtype: str
        """
        return self._tracking_id

    @tracking_id.setter
    def tracking_id(self, tracking_id):
        """Sets the tracking_id of this SpdTrackingItem.

        The tracking ID associated with each box in a non-Amazon partnered Small Parcel Delivery (SPD) shipment.  # noqa: E501

        :param tracking_id: The tracking_id of this SpdTrackingItem.  # noqa: E501
        :type: str
        """
        if tracking_id is not None and len(tracking_id) > 1024:
            raise ValueError("Invalid value for `tracking_id`, length must be less than or equal to `1024`")  # noqa: E501
        if tracking_id is not None and len(tracking_id) < 1:
            raise ValueError("Invalid value for `tracking_id`, length must be greater than or equal to `1`")  # noqa: E501

        self._tracking_id = tracking_id

    @property
    def tracking_number_validation_status(self):
        """Gets the tracking_number_validation_status of this SpdTrackingItem.  # noqa: E501

        Indicates whether Amazon has validated the tracking number. Because shipment validation is asynchronous, tracking IDs might not be validated immediately, and the status might change after a few hours. If more than 24 hours have passed and the status is not yet 'VALIDATED' or `NOT_SUPPORTED`, verify the number and update it if necessary. **Possible values:** `VALIDATED`, `NOT_VALIDATED`, `NOT_SUPPORTED` (Amazon is unable to find tracking information for the provided tracking ID).  # noqa: E501

        :return: The tracking_number_validation_status of this SpdTrackingItem.  # noqa: E501
        :rtype: str
        """
        return self._tracking_number_validation_status

    @tracking_number_validation_status.setter
    def tracking_number_validation_status(self, tracking_number_validation_status):
        """Sets the tracking_number_validation_status of this SpdTrackingItem.

        Indicates whether Amazon has validated the tracking number. Because shipment validation is asynchronous, tracking IDs might not be validated immediately, and the status might change after a few hours. If more than 24 hours have passed and the status is not yet 'VALIDATED' or `NOT_SUPPORTED`, verify the number and update it if necessary. **Possible values:** `VALIDATED`, `NOT_VALIDATED`, `NOT_SUPPORTED` (Amazon is unable to find tracking information for the provided tracking ID).  # noqa: E501

        :param tracking_number_validation_status: The tracking_number_validation_status of this SpdTrackingItem.  # noqa: E501
        :type: str
        """
        if tracking_number_validation_status is not None and len(tracking_number_validation_status) > 1024:
            raise ValueError("Invalid value for `tracking_number_validation_status`, length must be less than or equal to `1024`")  # noqa: E501
        if tracking_number_validation_status is not None and len(tracking_number_validation_status) < 1:
            raise ValueError("Invalid value for `tracking_number_validation_status`, length must be greater than or equal to `1`")  # noqa: E501

        self._tracking_number_validation_status = tracking_number_validation_status

    def to_dict(self):
        """Returns the model properties as a dict"""
        result = {}

        for attr, _ in six.iteritems(self.swagger_types):
            value = getattr(self, attr)
            if isinstance(value, list):
                result[attr] = list(map(
                    lambda x: x.to_dict() if hasattr(x, "to_dict") else x,
                    value
                ))
            elif hasattr(value, "to_dict"):
                result[attr] = value.to_dict()
            elif isinstance(value, dict):
                result[attr] = dict(map(
                    lambda item: (item[0], item[1].to_dict())
                    if hasattr(item[1], "to_dict") else item,
                    value.items()
                ))
            else:
                result[attr] = value
        if issubclass(SpdTrackingItem, dict):
            for key, value in self.items():
                result[key] = value

        return result

    def to_str(self):
        """Returns the string representation of the model"""
        return pprint.pformat(self.to_dict())

    def __repr__(self):
        """For `print` and `pprint`"""
        return self.to_str()

    def __eq__(self, other):
        """Returns true if both objects are equal"""
        if not isinstance(other, SpdTrackingItem):
            return False

        return self.__dict__ == other.__dict__

    def __ne__(self, other):
        """Returns true if both objects are not equal"""
        return not self == other
