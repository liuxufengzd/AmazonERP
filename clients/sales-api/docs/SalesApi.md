...
# **get_order_metrics**
> GetOrderMetricsResponse get_order_metrics(marketplace_ids, interval, granularity, granularity_time_zone=granularity_time_zone, buyer_type=buyer_type, fulfillment_network=fulfillment_network, first_day_of_week=first_day_of_week, asin=asin, sku=sku, amazon_program=amazon_program)
 ...
 **amazon_program** | **str**| Filters the results by the Amazon program that you specify. Do not include this filter if you want the response to include order metrics for all programs. **Example:** &#x60;AmazonHaul&#x60; returns order metrics for the Amazon Haul program only. | [optional] 
