...
# create an instance of the API class
api_instance = swagger_client.VendorOrdersApi()
limit = 789 # int | The limit to the number of records returned. Default value is 100 records. (optional)
...
include_details = 'include_details_example' # str | When true, returns purchase orders with complete details. Otherwise, only purchase order numbers are returned. Default value is true. (optional)
...
is_po_changed = 'is_po_changed_example' # str | When true, returns purchase orders which were modified after the order was placed. Vendors are required to pull the changed purchase order and fulfill the updated purchase order and not the original one. Default value is false. (optional)
 ...
 **include_details** | **str**| When true, returns purchase orders with complete details. Otherwise, only purchase order numbers are returned. Default value is true. | [optional] 
 ...
 **is_po_changed** | **str**| When true, returns purchase orders which were modified after the order was placed. Vendors are required to pull the changed purchase order and fulfill the updated purchase order and not the original one. Default value is false. | [optional] 
...
# create an instance of the API class
api_instance = swagger_client.VendorOrdersApi()
body = swagger_client.SubmitAcknowledgementRequest() # SubmitAcknowledgementRequest | Submits acknowledgements for one or more purchase orders from a vendor.
